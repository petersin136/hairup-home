...
- `07_Template Collection` 카드 줄 — 가운데 한 장만 800 × 500 으로 크고 좌우 이웃은
  ...
  걸쳐 있는 카드를 눌러 가운데로 가져올 수 있고, 목록을 세 벌 이어 붙여 어느 쪽으로든
  끝없이 돌아갑니다.
- `07` 카드 호버 — 대문 이미지가 4% 확대되면서 60% 어두워지고 템플릿 이름과
  VIEW DEMO 버튼이 떠오릅니다. 버튼에 호버하면 테두리만 있던 것이 크림색으로 채워지고,
  누르면 해당 데모 사이트가 새 탭에서 열립니다.
- `07` 하단 문구 띠 — 문구 목록을 두 벌 이어 붙여 42초에 한 바퀴, 오른쪽에서 왼쪽으로
